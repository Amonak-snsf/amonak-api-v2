...
export class SellerInfosService {
  ...
  // Méthode pour gérer le statut des informations du vendeur
  async manageSellerInfoStatus(user: string, status, res) {
    await put(this.sellerInforModel, { status: status }, { user: user });  // Met à jour le statut des infos du vendeur
    const account = this.status(status);  // Récupère le type de compte basé sur le statut
    await put(this.userModel, { accountType: account }, { user: user });  // Met à jour le type de compte utilisateur
    return res.status(HttpStatus.OK).json({ message: "Le statut du compte utilisateur a été changé en " + this.accountType + " avec succès!" });  // Renvoie un message de succès
  }
