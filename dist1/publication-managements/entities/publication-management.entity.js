"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.PubManagementSchema = exports.PublicationManagement = void 0;
const mongoose = require("@nestjs/mongoose");
const publication_entity = require("../../publications/entities/publication.entity");
const mongoose = require("mongoose");
const user_entity = require("../../users/entities/user.entity");
const default_model = require("../../utils/default-model");
class PublicationManagement extends default_model.DefaultModel {
}
__decorate([
    (0, mongoose.Prop)({ required: true, type: mongoose.Schema.Types.ObjectId, ref: 'Publication' }),
    __metadata("design:type", publication_entity.Publication)
], PublicationManagement.prototype, "publication", void 0);
__decorate([
    (0, mongoose.Prop)({ required: true, type: mongoose.Schema.Types.ObjectId, ref: 'User' }),
    __metadata("design:type", user_entity.User)
], PublicationManagement.prototype, "user", void 0);
__decorate([
    (0, mongoose.Prop)({ required: false, type: String }),
    __metadata("design:type", String)
], PublicationManagement.prototype, "type", void 0);
__decorate([
    (0, mongoose.Prop)({ required: false, type: String }),
    __metadata("design:type", String)
], PublicationManagement.prototype, "reason", void 0);
__decorate([
    (0, mongoose.Prop)({ required: false, type: Boolean, default: true }),
    __metadata("design:type", String)
], PublicationManagement.prototype, "status", void 0);
exports.PublicationManagement = PublicationManagement;
exports.PubManagementSchema = mongoose.SchemaFactory.createForClass(PublicationManagement);
//# sourceMappingURL=publication-management.entity.js.map