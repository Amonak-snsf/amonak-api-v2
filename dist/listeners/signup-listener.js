...
let SignupListener = class SignupListener {
    ...
    handleSignupEvent(user) {
        ...
        new this.biographyModel({ user_id: user._id }).save();
        new this.sellerInfoModel({ user_id: user._id, status: status_seller_info_1.Status.accepted }).save();
    }
    saveSignup(data) {
        this.sendUserConfirmation(data);
        new this.biographyModel({ user_id: data._id }).save();
        new this.sellerInfoModel({ user_id: data._id, status: status_seller_info_1.Status.accepted }).save();
    }
    sendUserConfirmation(user) {
        const token = Math.floor(1000 + Math.random() * 9000).toString();
        const url = `${this.configService.get('front_url')}/auth/activation`;
        new this.tokenModel({ token: token, user_id: user._id }).save();
